...
export default function ProgressiveImage({ src, alt = '', className = '', ...rest }: ProgressiveImageProps) {
    ...
    return (
        <div className={`relative overflow-hidden bg-gradient-to-r from-[#0f1720] via-[#0b1220] to-[#0f1720] ${className}`.trim()}>
            {/* Placeholder overlay: mantém gradiente visível enquanto carrega */}
            <div
                aria-hidden
                className={`absolute inset-0 transition-opacity duration-700 ease-out ${loaded ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}
